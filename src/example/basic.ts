...
export default class ExampleBasic {
    ...
    public static node_main(process: NodeJS.Process) : Promise<void> {
        return new Promise(async resolve => {
            ...
            let crc = this.decimalToHexString(LibDeflate.crc32(data));
            console.log(`Read input file ${file} ok, size=${data.byteLength} bytes, CRC-32=${crc}`);
        
            let gzipped = await this.gzip(data);
            console.log(`Gzip finished, compressed file size=${gzipped.byteLength} bytes`);
            fs.writeFileSync(file + ".gz", gzipped);
        
            let ungzipped = await this.ungzip(gzipped);
            let new_crc = this.decimalToHexString(LibDeflate.crc32(ungzipped));
    ...
    public static gzip(data: Uint8Array) : Promise<Uint8Array> {
    ...
    public static ungzip(data: Uint8Array) : Promise<Uint8Array> {
    ...
    public static decimalToHexString(number: number) : string {
        if (number < 0) {
